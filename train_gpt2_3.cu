...
// Communication timing infrastructure for multi-node scenarios
typedef struct {
    cudaEvent_t nccl_start;      // Start of NCCL operation
    cudaEvent_t nccl_end;        // End of NCCL operation
    cudaEvent_t compute_end;     // End of compute before communication
    bool initialized;
} CommTimingEvents;

static CommTimingEvents comm_timing = {0};



void init_communication_timing() {
    if (!comm_timing.initialized) {
        cudaCheck(cudaEventCreate(&comm_timing.nccl_start));
        cudaCheck(cudaEventCreate(&comm_timing.nccl_end)); 
        cudaCheck(cudaEventCreate(&comm_timing.compute_end));
        comm_timing.initialized = true;
    }
}

void cleanup_communication_timing() {
    if (comm_timing.initialized) {
        cudaCheck(cudaEventDestroy(comm_timing.nccl_start));
        cudaCheck(cudaEventDestroy(comm_timing.nccl_end));
        cudaCheck(cudaEventDestroy(comm_timing.compute_end));
        comm_timing.initialized = false;
    }
}

// Start timing NCCL operations - call before NCCL group
void start_nccl_timing(cudaStream_t nccl_stream, cudaStream_t compute_stream) {
    if (multi_gpu_config.num_processes == 1) {
        return; // No communication needed for single GPU
    }
    
    // Synchronize compute stream to ensure accurate timing
    cudaCheck(cudaStreamSynchronize(compute_stream));
    
    // Record start of NCCL operations on NCCL stream
    cudaCheck(cudaEventRecord(comm_timing.nccl_start, nccl_stream));
}

// End timing NCCL operations - call after NCCL group completes
float end_nccl_timing(cudaStream_t nccl_stream) {
    if (multi_gpu_config.num_processes == 1) {
        return 0.0f;
    }
    
    // Record end of NCCL operations
    cudaCheck(cudaEventRecord(comm_timing.nccl_end, nccl_stream));
    
    // Synchronize to ensure NCCL operations are complete
    cudaCheck(cudaEventSynchronize(comm_timing.nccl_end));
    
    // Calculate elapsed time
    float comm_time_ms;
    cudaCheck(cudaEventElapsedTime(&comm_time_ms, comm_timing.nccl_start, comm_timing.nccl_end));
    
    return comm_time_ms;
}

// Enhanced gradient reduction with timing
template<int N>
float multi_gpu_async_reduce_gradient_timed(
        floatX* const (&pointers)[N], const size_t (&pointers_sizes)[N],
        MultiGpuConfig* config, cudaStream_t compute_stream) {
    if (config->num_processes == 1) {
        return 0.0f; // no multi-GPU, just exit.
    }
    ...
    start_nccl_timing(config->nccl_stream, compute_stream);
    ...
    // End timing and return communication time
    return end_nccl_timing(config->nccl_stream);
#else
    return 0.0f;
#endif
}

// Global communication metrics tracker for detailed logging
typedef struct {
    float gradient_reduce_time_ms;     // Time for gradient AllReduce/ReduceScatter
    float weight_allgather_time_ms;    // Time for weight AllGather in ZeRO-1
    float loss_reduce_time_ms;         // Time for loss reduction
    float total_pure_comm_time_ms;     // Total pure NCCL communication time
    int num_gradient_reductions;       // Number of gradient reduction operations
    int num_weight_allgathers;         // Number of weight AllGather operations
} DetailedCommMetrics;

static DetailedCommMetrics global_comm_metrics = {0};

void reset_communication_metrics() {
    global_comm_metrics.gradient_reduce_time_ms = 0.0f;
    global_comm_metrics.weight_allgather_time_ms = 0.0f;
    global_comm_metrics.loss_reduce_time_ms = 0.0f;
    global_comm_metrics.total_pure_comm_time_ms = 0.0f;
    global_comm_metrics.num_gradient_reductions = 0;
    global_comm_metrics.num_weight_allgathers = 0;
}

void update_gradient_comm_metrics(float time_ms) {
    global_comm_metrics.gradient_reduce_time_ms += time_ms;
    global_comm_metrics.total_pure_comm_time_ms += time_ms;
    global_comm_metrics.num_gradient_reductions++;
}

void update_allgather_comm_metrics(float time_ms) {
    global_comm_metrics.weight_allgather_time_ms += time_ms;
    global_comm_metrics.total_pure_comm_time_ms += time_ms;
    global_comm_metrics.num_weight_allgathers++;
}

void update_loss_comm_metrics(float time_ms) {
    global_comm_metrics.loss_reduce_time_ms += time_ms;
    global_comm_metrics.total_pure_comm_time_ms += time_ms;
}

// Diagnostic function to help identify communication bottlenecks
void print_nccl_diagnostics() {
    if (multi_gpu_config.process_rank == 0) {
        printf0("=== NCCL Communication Diagnostics ===\n");
        printf0("Number of processes: %d\n", multi_gpu_config.num_processes);
        printf0("Zero stage: %d\n", multi_gpu_config.zero_stage);
        printf0("Shard parameters: %zu\n", multi_gpu_config.shard_num_parameters);
        
        // Calculate approximate data sizes for communication
        size_t gradient_data_size = multi_gpu_config.shard_num_parameters * sizeof(floatX);
        printf0("Approximate gradient data per AllReduce: %.2f MB\n", 
                (float)gradient_data_size / (1024.0f * 1024.0f));
        
        if (multi_gpu_config.zero_stage == 1) {
            printf0("ZeRO-1 AllGather data per parameter tensor: varies by layer\n");
        }
        printf0("=======================================\n");
    }
}
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        if(last_step) {
            ...
            float gradient_comm_time = multi_gpu_async_reduce_gradient_timed(pointers, nelem, &multi_gpu_config, main_stream);
            update_gradient_comm_metrics(gradient_comm_time);
        }
    }
    ...
    if(last_step) {
        ...
        #if MULTI_GPU
        start_nccl_timing(multi_gpu_config.nccl_stream, main_stream);
        ncclCheck(ncclAllReduce(model->accumulated_mean_loss, model->accumulated_mean_loss, sizeof(float), ncclFloat, ncclAvg, multi_gpu_config.nccl_comm, multi_gpu_config.nccl_stream));
        float loss_comm_time = end_nccl_timing(multi_gpu_config.nccl_stream);
        update_loss_comm_metrics(loss_comm_time);
        #endif
        ...
        float final_gradient_comm_time = multi_gpu_async_reduce_gradient_timed(pointers, nelem, &multi_gpu_config, main_stream);
        update_gradient_comm_metrics(final_gradient_comm_time);
    }
...
void log_metrics_to_csv(const char* filename, int step, float loss, float computation_time_ms, float communication_time_ms, float total_time_ms, float gflops_per_sec, const DetailedCommMetrics* comm_metrics) {
    ...
    if (ftell(f) == 0) {
        fprintf(f, "step,loss,computation_time_ms,communication_time_ms,total_time_ms,gflops_per_sec,"
                   "gradient_reduce_time_ms,loss_reduce_time_ms,total_pure_comm_time_ms,num_gradient_reductions\n");
    }
    fprintf(f, "%d,%f,%f,%f,%f,%f,%f,%f,%f,%d\n", 
            step, loss, computation_time_ms, communication_time_ms, total_time_ms, gflops_per_sec,
            comm_metrics->gradient_reduce_time_ms, comm_metrics->loss_reduce_time_ms, 
            comm_metrics->total_pure_comm_time_ms, comm_metrics->num_gradient_reductions);
    fclose(f);
}
...
int main(int argc, char *argv[]) {
    ...
    // Initialize communication timing infrastructure for accurate multi-node measurements
    init_communication_timing();
    ...
    // Print communication diagnostics for multi-node setups
    if (multi_gpu_config.num_processes > 1) {
        print_nccl_diagnostics();
    }
    ...
    for (; step <= train_num_batches; step++) {
        ...
        // Reset communication metrics for this training step
        ...
        for (int micro_step = 0; micro_step < grad_accum_steps; micro_step++) {
            ...
            float forward_time;
            cudaCheck(cudaEventElapsedTime(&forward_time, comm_start, comm_end));
            computation_time_ms += forward_time;

            // Backward pass - this will include gradient communication timing
            gpt2_backward_and_reduce(&model, train_loader.inputs, train_loader.targets, grad_accum_steps, micro_step);
            
            // Note: Communication time is now measured inside gpt2_backward_and_reduce using our new timed functions
        }
        ...
        // Note: computation_time_ms now includes both compute and communication
        // Pure communication time is tracked separately via our enhanced timing functions
        ...
        // Calculate communication time from total - computation
        float communication_time_ms = total_time_ms - computation_time_ms;
        ...
        // Print detailed communication breakdown for multi-GPU scenarios
        if (multi_gpu_config.num_processes > 1 && multi_gpu_config.process_rank == 0) {
            printf0("         comm breakdown: grad_reduce=%.2fms loss_reduce=%.2fms pure_nccl=%.2fms (grad_ops: %d)\n",
                    global_comm_metrics.gradient_reduce_time_ms, global_comm_metrics.loss_reduce_time_ms,
                    global_comm_metrics.total_pure_comm_time_ms, global_comm_metrics.num_gradient_reductions);
            
            // Calculate effective communication bandwidth
            if (global_comm_metrics.total_pure_comm_time_ms > 0.0f) {
                size_t gradient_data_size = multi_gpu_config.shard_num_parameters * sizeof(floatX);
                float total_data_mb = ((float)gradient_data_size * global_comm_metrics.num_gradient_reductions) / (1024.0f * 1024.0f);
                float bandwidth_gbps = (total_data_mb * 8.0f) / (global_comm_metrics.total_pure_comm_time_ms * 1000.0f);
                printf0("         comm bandwidth: %.2f MB transferred, effective %.2f Gbps\n", total_data_mb, bandwidth_gbps);
            }
        }
        
        if (output_log_dir != NULL) {
            log_metrics_to_csv(metrics_filename, step + 1, model.mean_loss, computation_time_ms, communication_time_ms, total_time_ms, gflops_per_sec, &global_comm_metrics);
        }
    ...
    // Cleanup communication timing infrastructure
    cleanup_communication_timing();
